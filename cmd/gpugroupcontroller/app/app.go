...
import (
	...
	"github.com/kai-scheduler/KAI-scheduler/pkg/gpugroupcontroller/controllers"
)
...
func Run(options *Options, config *rest.Config, ctx context.Context) error {
	...
	mgr, err := ctrl.NewManager(config, ctrl.Options{
		...
		LeaderElectionID:       "gpugroup.kai.scheduler",
	})
	if err != nil {
	...
	if options.EnableGPUGroupWebhook {
		if err = (&kaiv1alpha1.GPUGroup{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook for GPUGroup")
			return err
		}
	}
